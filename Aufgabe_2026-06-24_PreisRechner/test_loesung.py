...
def calculate_net_payment(amounts: List[float], flat_fee: float, commission_rate: float) -> float:
    ...
    if amounts is None:
        raise InvalidInputException("Amounts list cannot be None.")
    
    if flat_fee is None:
        raise InvalidInputException("Flat fee cannot be None.")
        
    if commission_rate is None:
        raise InvalidInputException("Commission rate cannot be None.")

    # Überprüfung auf nicht-numerische Werte in der Liste
    for amount in amounts:
        if not isinstance(amount, (int, float)):
            raise InvalidInputException("All amounts must be numeric.")
        if amount < 0:
            raise InvalidInputException("Amounts must not be negative.")

    # Überprüfung der Pauschalgebühr
    if not isinstance(flat_fee, (int, float)):
        raise InvalidInputException("Flat fee must be numeric.")
    if flat_fee < 0:
        raise InvalidInputException("Flat fee must not be negative.")

    # Überprüfung des Prozentsatzes
    if not isinstance(commission_rate, (int, float)):
        raise InvalidInputException("Commission rate must be numeric.")
    if commission_rate < 0 or commission_rate > 100:
        raise InvalidInputException("Commission rate must be between 0 and 100.")

    # Aggregation der Einzelposten
    total_amount = sum(amounts)

    # Subtraktion der Pauschalgebühr
    remaining_amount = total_amount - flat_fee

    # Sicherstellen, dass der verbleibende Betrag nicht negativ ist
    if remaining_amount < 0:
        remaining_amount = 0.0

    # Anwendung des Prozentsatzes (Provisionssatz)
    net_payment = remaining_amount * (1 - commission_rate / 100)
